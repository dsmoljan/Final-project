...
def validation(args):
    ...
    if (args.model == 'supervised_model'):
        ...
        for i, (image_test, real_segmentation, image_name) in enumerate(val_loader):
            ...
            if args.dataset == 'voc2012' or args.dataset == 'cityscapes' or args.dataset == 'ortopanograms':
                trans_mean = [0.5, 0.5, 0.5]
                trans_std = [0.5, 0.5, 0.5]
                for k in range(3):
                    fake_img[:, k, :, :] = ((fake_img[:, k, :, :] * trans_std[k]) + trans_mean[k])
                    fake_img_from_labels[:, k, :, :] = (
                                (fake_img_from_labels[:, k, :, :] * trans_std[k]) + trans_mean[k])
            ...
            for j in range(prediction.shape[0]):
                ...
                ### Now the new_img is PIL.Image
                new_img.save(
                    os.path.join(args.validation_dir + '/unsupervised/generated_labels/' + image_name[j] + '.png'))
                regen_label.save(
                    os.path.join(args.validation_dir + '/unsupervised/regenerated_labels/' + image_name[j] + '.png'))
                torchvision.utils.save_image(fake_img[j], os.path.join(
                    args.validation_dir + '/unsupervised/regenerated_image/' + image_name[j] + '.jpg'))
                torchvision.utils.save_image(fake_img_from_labels[j], os.path.join(
                    args.validation_dir + '/unsupervised/image_from_labels/' + image_name[j] + '.jpg'))
