...
class supervised_model(object):
    ...
    def train(self, args):
        ...
        transform = get_transformation((self.args.crop_height, self.args.crop_width), resize=True, dataset=args.dataset) #ovo je dio koda koji ti pretvara to u tenzor, samo idi vidi što je točno get_transformation
...
class semisuper_cycleGAN(object):
    ...
    def train(self, args):
        transform = get_transformation((args.crop_height, args.crop_width), resize=True, dataset=args.dataset) #tu se postavlja tražena transformacija (npr. 200x200)
        ...
        for epoch in range(self.start_epoch, args.epochs):
            ...
            for i, ((l_img, l_gt, _), (unl_img, _)) in enumerate(zip(labeled_loader, unlabeled_loader)):
                ...
                ##################################################
                fake_img = self.Gis(make_one_hot(l_gt, args.dataset, args.gpu_ids).float()) #mislim da ovdje make_one_hot iz osnovne slike radi novi vektor sa odgovarajućim dimemnzijama, znači u svojem kodu ćeš to morati maknuti
            ...
            val_image, val_gt, _ = iter(val_loader).next() #samo učitava sliku iz validation dataseta i njen ground truth, dataloader ih vraća kao tenzore
