...
acdc_palette = [0, 0, 0, 128, 64, 128, 70, 70, 70, 250, 170,
                30]  # iz pocetnih 0,0,0 vidimo da je pozadina sigurno jedna od klasa u modelu
...
ortopanograms_palette_thirtythree_classes = [0, 0, 0, 147, 58, 22, 141, 2, 31, 94, 25, 20, 184, 15, 10, 66, 13, 9, 255,
                                             ...
                                             255, 253, 55, 255, 239, 0, 253, 255, 0, 210, 181, 91, 238, 220, 130, 238,
                                             237, 9, 250, 218, 94, 255, 215, 0, 238, 210, 2, 232, 172, 65, 255, 253, 1,
                                             238, 234, 98, 207, 255, 4, 213, 182, 10, 226, 202, 118, 255, 218, 3]
...
zero_pad = 256 * 3 - len(ortopanograms_palette_two_classes)
for i in range(zero_pad):
    ortopanograms_palette_two_classes.append(0)

zero_pad = 256 * 3 - len(ortopanograms_palette_three_classes)
for i in range(zero_pad):
    ortopanograms_palette_three_classes.append(0)

zero_pad = 256 * 3 - len(ortopanograms_palette_thirtythree_classes)
for i in range(zero_pad):
    ortopanograms_palette_thirtythree_classes.append(0)


def colorize_mask(mask, dataset, ortopanograms_classes=2):
    '''
    ...
    if (dataset == 'voc2012'):
        ...
        if (ortopanograms_classes == 2):
            new_mask.putpalette(ortopanograms_palette_two_classes)
        elif (ortopanograms_classes == 3):
            new_mask.putpalette(ortopanograms_palette_three_classes)
        elif (ortopanograms_classes == 33):
            new_mask.putpalette(ortopanograms_palette_thirtythree_classes)
...
def PIL_to_tensor(img, dataset, ortopanograms_classes=2):
    '''
    ...
    if (dataset == 'voc2012'):
        ...
        if (ortopanograms_classes == 2):
            ortopanograms_palette = ortopanograms_palette_two_classes
        elif (ortopanograms_classes == 3):
            ortopanograms_palette = ortopanograms_palette_three_classes
        elif (ortopanograms_classes == 33):
            ortopanograms_palette = ortopanograms_palette_thirtythree_classes
...
def smoothen_label(label, alpha, gpu_id):
    ...
    torch.manual_seed(0)
    try:
        smoothen_array = -1 * alpha + torch.rand([label.shape[0], label.shape[1], label.shape[2], label.shape[3]]) * (
                    2 * alpha)
...
def make_one_hot(labels, dataname, gpu_id, ortopanograms_classes = 2):
    '''
    ...
    '''
    assert dataname in ('voc2012', 'cityscapes', 'acdc',
                        'ortopanograms'), 'dataset name should be one of the following: \'voc2012\',given {}'.format(
        dataname)
    ...
    try:
        ...
        one_hot = torch.FloatTensor(labels.size(0), C, labels.size(2), labels.size(
            3)).zero_()  # https://stackoverflow.com/questions/55565687/i-dont-understand-the-code-for-training-a-classifier-in-pytorch obja�njava �to znaci npr. labels(0)
...
class runningScore(object):
    ...
    def get_scores(self):
        ...
        if self.dataset == 'voc2012':
            iu = np.diag(hist[1:self.n_classes, 1:self.n_classes]) / (
                        hist[1:self.n_classes, 1:self.n_classes].sum(axis=1) + hist[1:self.n_classes,
                                                                               1:self.n_classes].sum(axis=0) - np.diag(
                    hist[1:self.n_classes, 1:self.n_classes]))
        elif self.dataset == 'cityscapes':
            iu = np.diag(hist[0:self.n_classes - 1, 0:self.n_classes - 1]) / (
                        hist[0:self.n_classes - 1, 0:self.n_classes - 1].sum(axis=1) + hist[0:self.n_classes - 1,
                                                                                       0:self.n_classes - 1].sum(
                    axis=0) - np.diag(hist[0:self.n_classes - 1, 0:self.n_classes - 1]))
