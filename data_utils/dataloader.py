    ...
    def __init__(self, root_path, name='label', ratio=1, transformation=None, augmentation=None): #ratio je valjda koliko od training slika da uzme za "neoznačene"?
        ...
        if self.name != 'test':
            ...
            if self.ratio > 1:
                ...
                new_imgs = np.random.choice(unlabeled_imgs,  size = int(len(labeled_imgs) - len(unlabeled_imgs)), replace=False)
                new_list = list(new_imgs)
    ...
    def __getitem__(self, index):
        if self.name == 'test':
            ...
            for i in range(len(ora_image['root']['childs'])):
                ...
                #spajamo samo segmentacijske mape zubiju te pozadinu u jednu sliku/tensor -> samo one imaju imena duljine točno 2
                if (len(name) == 2):
...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.name == 'test':
            img_path = os.path.join(self.root_path, 'JPEGImages', self.imgs[index] + '.jpg') #pošto se radi o test datasetu, učitavamo samo .jpg, odnosno sliku, bez njene segmentacijske maske
            ...
            gt_path = os.path.join(self.root_path, 'SegmentationClassAug', self.gts[index] + '.png') #drugo je ručno označena maska
